...
INT WINAPI wWinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPWSTR lpCmdLine, _In_ INT nCmdShow){
    ...
    std::unique_ptr<Game> game = std::make_unique<Game>(L"2.5D Camera");
    ...
    XMFLOAT4 color_red;
    XMStoreFloat4(&color_red, Colors::IndianRed);
    std::shared_ptr<PointLight> pointLight_red = std::make_shared<PointLight>(XMFLOAT4(-5.77f, 5.77f, -5.77f, 1.0f), color_red);

    if (FAILED(game->GetRenderer()->AddPointLight(1u, pointLight_red)))
        return 0;
  
    std::shared_ptr<Character> stone = std::make_shared<Character>(L"../../Data/Stone/Stone.obj");
